...
p1 <- ggplot(data = data, aes(x = HI, y = Health, color = Region)) 
...
p4 <- p3 +  scale_x_continuous(name = "Happiness Index") + 
  scale_y_continuous(name = "Health") 
...
p5 <- p5 + ggtitle ("Happiness Index and Health")
p6 <- p5 + theme_minimal() +
...
mR2 <- summary(lm(Health ~ log(HI), data = data))$r.squared

png(file = "healthandhappiness.png", width = 800, height = 600)
p6 
