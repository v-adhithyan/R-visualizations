  ...
  p1 <- ggplot(data = data, aes(x = HI, y = Freedom, color = Region)) 
  ...
  p4 <- p3 +  scale_x_continuous(name = "Happiness Index") + 
    scale_y_continuous(name = "Freedom") 
  ...
  p5 <- p5 + ggtitle ("Happiness Index and Freedom") + coord_flip()
  p6 <- p5 + theme_minimal() +
  ...
  mR2 <- summary(lm(Freedom ~ log(HI), data = data))$r.squared
  
  png(file = "freedom.png", width = 800, height = 600)
  p6 
